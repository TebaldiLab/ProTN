...
doNextChunk <- tryCatch({
  ...
  enrich_df<-enr_df[which(enr_df$Significative == "TRUE" & enr_df$overlap_size>=overlap_size_enrich_thr),]
...
if(length(enrich_filter_DBs) > 0){
  ...
  enr_sele_df <- enr_sele_df %>% dplyr::filter(!str_ends(input_name,"_all"))
  enr_sele_df$Direction <- stri_extract_last_regex(str = enr_sele_df$input_name, pattern = "_up$|_down$|_all$")
  enr_sele_df$Direction <- str_c(enr_sele_df$Significative, enr_sele_df$Direction)
  knitr::opts_chunk$set(fig.height = max(min(20,length(unique(enr_sele_df$anno_name))*0.5),3))
}
```
...
if(length(enrich_filter_DBs) > 0){
  ...
  names(col_vec)<-unique(enr_sele_df$input_name)
  # }
  el<- enrichment_dotmatrix(input_df=enr_sele_df,
                           size_col="log2_OR",
                           color_col="input_name",
                           color_vec = col_vec,
                           shape_vec=c("FALSE_down"=6,"FALSE_up"=2,"TRUE_down"=25,"TRUE_up"=17,"TRUE_all"=16, "FALSE_all"=1),
                           shape_col = "Direction",
                           fill_col = "Significative",
                           fill_vec=c("grey25","white"),
                           # color_vec=color_base[sort(rep(c(1:length(color_base)),2))],
                           char_max=60) +guides(color=F, shape=guide_legend(override.aes = list(size=4)), fill=F)
...
if(p$is_alive() | is.null(p$get_result())){
  ...
  p$kill()
  #Save in RData for possible further analysis
  enrich_all_df <- enr_df_universe %>% dplyr::filter(Significative == "TRUE", overlap_size>=overlap_size_enrich_thr)
...
# library(pdftools)
# library(magick)
# image_read_pdf(paste0(dirOutput_net,"LRRvsAll","_network.pdf"))
# knitr::include_graphics(path = paste0(dirOutput_net,names(doComp)[1],"_network.pdf"))
# knitr::include_graphics(path = paste0(dirOutput_net,"LRRvsAll","_network.pdf"))
