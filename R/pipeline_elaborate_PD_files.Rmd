...
if(deps_l_df %>% dplyr::filter(class!="=") %>% nrow() > 0){
  ...
  #   cat(oldheig)
  #   knitr::opts_chunk$set(fig.height = unit(max(min(20,length(heig)*0.75),2),"in"))
  #   cat("\n \n ")
  #   cat(knitr::opts_chunk$get("fig.height"))
  # }
  cat("##### Proteins \n")
  print(pDEPs)
  cat(" \n \n")
...
if(length(enrich_filter_DBs) > 0){
  ...
  category_db <- lapply(dbs_category, function(x){filter(x, x[,1] %in% enrich_filter_DBs)})
  # names(category_db)<-paste0(names(category_db),"_DB_ALL")
  ...
  tryCatch({
    ...
    for(db in names(plotlist)){
      knitr::opts_chunk$set(fig.height = max(min(20,length(unique(enr_sele_df[which(enr_sele_df$anno_class %in% category_db[[db]][,1]),
                                                                              "anno_name"]))*0.5),3))
      cat(paste0("##### ",db," \n"))
      suppressWarnings(print(plotlist[[db]]))
      cat(" \n \n")
    }
    # suppressWarnings(print(in_tabs(plotlist, labels = names(plotlist), level = 4L)))
    cat(" \n \n")
    name_list<-vector()
    for(db in names(plotlist)){
    ...
    # ggsave(paste0(dirOutput_enr,"enr_DE_keysources.pdf"), el, 
    #        device=cairo_pdf, width = 7, 
    #        height = max(min(20,length(unique(enr_sele_df$anno_name))*0.5),3), units = c("in"))
  },
  error=function(cond){
...
if(length(enrich_filter_DBs) > 0){
  ...
  # if(length(as.vector(t(color_constrast)))>0){
  category_db <- lapply(dbs_category, function(x){filter(x, x[,1] %in% enrich_filter_DBs)})
  # names(category_db)<-paste0(names(category_db),"_DB")
...
if(length(enrich_filter_term) > 0){
  ...
  # names(category_db)<-paste0(names(category_db),"_TERM_ALL")
...
if(length(enrich_filter_term) > 0){
  ...
  # if(length(as.vector(t(color_constrast)))>0){
  category_db <- lapply(dbs_category, function(x){filter(x, x[,1] %in% unique(enr_sele_df$anno_class))})
  # names(category_db)<-paste0(names(category_db),"_TERM")
