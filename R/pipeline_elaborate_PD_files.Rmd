...
```{r print_descr_enrich, warning=FALSE, echo=F, message=FALSE, eval=run_enrich_universe, results='asis'}
cat("- ```DEPs_enrichment_data_universe.RData```: result of the enrichment made with EnrichR \n")
...
cat("- ```enrich``` directory: plots of the enrichment data under the parameters set \n")  
cat("  - **enrichment_database_filter_universe.pdf**: dot matrix of the enrichment result divided in up- and down-regulated genes. Filtered by the datasets choose in the advanced options. \n")
cat("  - **enrichment_terms_filter_universe.pdf**: dot matrix of the enrichment result divided in up- and down-regulated genes. Filtered by the terms written in the advanced options. \n")
cat("  - **enrichment_database_filter_all_universe.pdf**: dot matrix of the enrichment result combining the up- and down- regulated genes in a single list. Filtered by the datasets choose in the advanced options. \n")
cat("  - **enrichment_terms_filter_all_universe.pdf**: dot matrix of the enrichment result combining the up- and down- regulated genes in a single list. Filtered by the terms written in the
    advanced options. \n")
...
df_stat <- data.frame("number" = c(length(unique(psm_anno_df$symbol)), nrow(psm_anno_df)), row.names = c("Discovered proteins:", "Discovered peptides:"))
kable(df_stat)
```
...
cat("Following are visualized the Principal Component analysis (PCA) of the normalized data. Each sample has its own color described in the Input file, if it was not defined it is used the default palette coloring by condition. \n")
...
shiny::setProgress(0.70, detail = "Perform PCA...")
  cat(" \n \n")
  cat("#### 3.3. PCA analysis of samples based on up-/down-regulated peptides {.tabset}  \n")
  cat("Following are visualized the Principal Component analysis (PCA) of the normalized data. Each sample has its own color described in the Input file, if it was not defined it is used the default palette coloring by condition. \n")
  cat("Two PCA are created, one based on the normalized protein intensities, tab **Proteins**. The second on the normalized peptide intensities, tab **Peptides** \n")
  cat(" \n \n")
...
shiny::setProgress(0.75, detail = "Done Enrichment...")
