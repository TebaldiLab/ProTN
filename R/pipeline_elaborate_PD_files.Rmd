...
list.of.packages <- c("ggrepel","tidyr","ggraph","graphlayouts","RColorBrewer",
                      ...
                      "tibble", "dplyr", "tidyr", "wesanderson","WGCNA","ggbeeswarm")
...
# save(dat_gene, c_anno, file = "/home/gabriele/Desktop/UNITN/Proteomics_internship/general_pipeline_PD_files/lib/tpm.RData")
...
suppressMessages(ggsave(paste0(dirOutput,"general_PCA_proteins.pdf"), p1, device=cairo_pdf, width = 4, height = 3, units = c("in")))
...
suppressMessages(ggsave(paste0(dirOutput,"general_PCA_peptides.pdf"), p1, device=cairo_pdf, width = 4, height = 3, units = c("in")))
...
if(length(prot_find)>0){
  ...
  prot_intensity_long$Condition <- unique((left_join(prot_intensity_long,
...
if(deps_l_df %>% dplyr::filter(class!="=") %>% nrow() > 0){
  ...
  # ncores <- min(length(unique(comp)), detectCores()-1)
  # plotVolcano <- function(comp){
  ...
  #   res<-append(list(),cmd)
  # }
  # volcano <- MASS::mcapply(unique(deps_l_df$comp), plotVolcano, mc.cores = ncores)
...
shiny::setProgress(0.70, detail = "Perform PCA...")
  ...
  suppressMessages(ggsave(paste0(dirOutput,"DEPs_proteins_PCA.pdf"), p1, device=cairo_pdf, width = 4, height = 3, units = c("in")))
...
if(deps_pep_l_df %>% dplyr::filter(class!="=") %>% nrow() > 0){
  ...
  suppressMessages(ggsave(paste0(dirOutput,"DEPs_peptides_PCA.pdf"), p1, device=cairo_pdf, width = 4, height = 3, units = c("in")))
