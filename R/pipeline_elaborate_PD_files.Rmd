...
#Export table with SE e MEAN
prot_id <- unique(psm_peptide_table[,c("Accession","GeneName")]) %>% group_by(GeneName) %>% summarize_all(toString) %>% left_join(
  unique(
    psm_peptide_table[,c("Accession","Description","GeneName")]), 
  by = "GeneName", 
  multiple = "first",
  suffix = c(".id", ".old"))
protein_DE <- unique(left_join(prot_id[(!duplicated(prot_id$Accession.id)),
                                       c("Accession.id","Description","GeneName")], 
                               toPrint, 
                               by=c("GeneName" = "GeneSymbol")))
