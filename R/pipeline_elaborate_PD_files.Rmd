...
- ```general_pca_proteins.pdf```: PCA of the samples by proteins.
- ```general_pca_peptides.pdf```: PCA of the samples by peptides.
...
- ```DEPs_proteins_count_barplot.pdf```: number of DEPs find for each contrast design based on proteins. Dark grey are the down-regulated, light grey are the up-regulated genes.
...
- ```DEPs_proteins_pca.pdf```: PCA of the DEPs discovered based on proteins.
- ```DEPs_peptides_pca.pdf```: PCA of the DEPs discovered based on peptides.
...
df_stat <- data.frame("number" = c(n_prot_preFILT, n_pep_pre_FILT, length(unique(psm_anno_df$symbol)), nrow(psm_anno_df)), 
                      row.names = c("Discovered proteins by the experiment - PRE FILTER:", "Discovered peptides by the experiment - PRE FILTER:", "Discovered proteins by the experiment:", "Discovered peptides by the experiment:"))
kable(df_stat)
```
...
psm_long_df <- psm_log_prot_df[,c_anno[order(c_anno$condition),"sample"]] %>% mutate(id=as.factor(rownames(psm_log_prot_df[,c_anno[order(c_anno$condition),"sample"]]))) %>% pivot_longer(-id, names_to = "sample", values_to = "log2_intensity")
psm_long_df <- suppressMessages(left_join(psm_long_df,c_anno))
...
hs<-ggplot(data=numeric_df, aes(x=factor(sample, levels=unique(psm_long_df$sample)),y=`Percentage of known intensity (%)`,fill=sample,colour=sample))+
  #geom_hline(yintercept = 0, lty=2)+
  coord_flip()+
...
#### 2.3. Normalization and Imputation 
...
readme_sheet <- data.frame("INFO"=c(   NA,
                                ...
                                NA,
                                "In sheet *Normalized proteins intensity* is written the normalized and imputed version of the original intensities of each protein in each sample.",
                                "In sheet *Normalized peptides intensity* is written the normalized and imputed version of the original intensities of each peptide in each sample.",
                                "In sheet *Normalized proteins group by condition* is written the average and the standard deviation of each condition described in the Input file for each protein. Where column *avg* is the average of the replicates, column *se* is the standard error, column *Coef_variant* is the coefficient of variant in %.",
                                "In sheet *Normalized peptides group by condition* is written the average and the standard deviation of each condition described in the Input file for each peptide. Where column *avg* is the average of the replicates, column *se* is the standard error, column *Coef_variant* is the coefficient of variant in %."
                              ))

#Export table with SE e MEAN
df_to_save<-list("README"=readme_sheet,
                 "Normalized proteins intensity"=df_to_save$`Normalized proteins matrix intensity`,
                 "Normalized peptides intensity"=merge(psm_peptide_table, dat_pep, by=0)[,-c(1)],
                 "Normalized proteins group by condition"=df_to_save$`Normalized proteins intensity group by condition`,
                 "Normalized peptides group by condition"=left_join(rownames_to_column(psm_peptide_table), 
...
pg_long_df <- dat_gene[,c_anno[order(c_anno$condition),"sample"]] %>% mutate(id=as.factor(rownames(dat_gene[,c_anno[order(c_anno$condition),"sample"]]))) %>% pivot_longer(-id, names_to = "sample", values_to = "intensity")
...
suppressMessages(ggsave(paste0(dirOutput,"general_mds_proteins.pdf"), cmd, device=cairo_pdf, width = (3+(0.13*nrow(mds_cmdscale))), height = (3+(0.07*nrow(mds_cmdscale))), units = c("in")))
...
suppressMessages(ggsave(paste0(dirOutput,"general_mds_peptides.pdf"), cmd, device=cairo_pdf, width = (3+(0.13*nrow(mds_cmdscale))), height = (3+(0.07*nrow(mds_cmdscale))), units = c("in")))
...
cat(" \n \n")
cat("#### 2.6. PCA analysis of samples {.tabset}  \n")
...
cat("Two PCA are created, one based on the normalized protein intensities, tab **Proteins**. The second on the normalized peptide intensities, tab **Peptides** \n")
cat(" \n \n")
...
suppressMessages(ggsave(paste0(dirOutput,"general_PCA_proteins.pdf"), p1, device=cairo_pdf, width = (3+(0.13*nrow(pc))), height = (3+(0.07*nrow(pc))), units = c("in")))
...
suppressMessages(ggsave(paste0(dirOutput,"general_PCA_peptides.pdf"), p1, device=cairo_pdf, width = (3+(0.13*nrow(pc))), height = (3+(0.07*nrow(pc))), units = c("in")))
...
cat("#### 2.7. Batch correction {.tabset}  \n")
...
suppressMessages(ggsave(paste0(dirOutput,"corrected_mds_proteins.pdf"), cmd, device=cairo_pdf, width = (3+(0.13*nrow(mds_cmdscale))), height = (3+(0.07*nrow(mds_cmdscale))), units = c("in")))
...
if(length(prot_find)>0){
  cat("#### 2.8. Control boxplot proteins intensities {.tabset}  \n")
  cat("Boxplot figures for the control of the intensities of selected proteins. For each protein shows the intensity group by condition. \n \n ")
  ...
  suppressMessages(ggsave(paste0(dirOutput,"boxplot_control_intensity.pdf"), g, device=cairo_pdf, 
...
deps_df <- limmafnc("PROT",c_anno,dat_gene,psm_count_table,formule_contrast,expr_avgse_df,signal_thr,fc_thr, pval_thr, pval_fdr)
...
deps_pep_df <- limmafnc("PEP",c_anno,dat_pep,psm_count_table,formule_contrast,expr_avgse_pep_df,signal_thr,fc_thr, pval_thr, pval_fdr)
...
readme_sheet <- data.frame("INFO"=c(   NA,
                                ...
                                "7. *class*: is “+” if the protein is up-regulated following the parameters set reported in the report.html, “-” if the protein is down-regulated, “=” otherwise",
...
#Export table with SE e MEAN
df_to_save<-list("README"=readme_sheet,
                 "DEPs_proteins"=left_join(psm_peptide_table, toPrint, by=c("GeneName" = "GeneSymbol")),
...
cat(" \n \n")
cat("#### Number of DEPs for each comparison {.tabset}  \n")
...
if(deps_l_df %>% dplyr::filter(class!="=") %>% nrow() > 0){
  ...
  suppressMessages(ggsave(paste0(dirOutput,"DEPs_proteins_count_barplot.pdf"), pDEPs, device=cairo_pdf, 
...
if(deps_pep_l_df %>% dplyr::filter(class!="=") %>% nrow() > 0){
  ...
  suppressMessages(ggsave(paste0(dirOutput,"DEPs_peptides_count_barplot.pdf"), pDEPs, device=cairo_pdf, 
                          height = max(min(20,length(heig)*0.75),2), units = c("in")))
  remove(pDEPs,cmd,mds_cmdscale)
}else{
  message("No gene differentiated discovered.")
...
if(deps_l_df %>% dplyr::filter(class!="=") %>% nrow() > 0){
  cat("#### 3.2. Multi-dimensional reduction analysis of samples based on up-/down-regulated proteins or peptides {.tabset}  \n " )
  cat("Following are visualized the MDS of the significative differentially expressed proteins/peptides. Each sample has its own color described in the Sample_Annotation file, if it was not defined it is used the default palette coloring by condition. \n")
  cat("Two MDS are created, one based on the normalized protein intensities, tab **Proteins**. The second on the normalized peptide intensities, tab **Peptides** \n")
  ...
  ggsave(paste0(dirOutput,"DEPs_proteins_mds.pdf"), cmd, device=cairo_pdf, width = (3+(0.13*nrow(mds_cmdscale))), height = (3+(0.07*nrow(mds_cmdscale))), units = c("in"))
...
if(deps_pep_l_df %>% dplyr::filter(class!="=") %>% nrow() > 0){
  ...
  ggsave(paste0(dirOutput,"DEPs_peptides_mds.pdf"), cmd, device=cairo_pdf, width = (3+(0.13*nrow(mds_cmdscale))), height = (3+(0.07*nrow(mds_cmdscale))), units = c("in"))

shiny::setProgress(0.69, detail = "Perform MDS...")
}
```

```{r MDS_deps_pep, warning=F, message=F, echo=F, eval=F, results='asis'}
if(deps_pep_l_df %>% dplyr::filter(class!="=") %>% nrow() > 0){
...
shiny::setProgress(0.70, detail = "Perform PCA...")
  cat(" \n \n")
  cat("#### 3.3. PCA analysis of samples based on up-/down-regulated proteins or peptides {.tabset}  \n")
  cat("Following are visualized the Principal Component analysis (PCA) of the differentially expressed proteins or peptides. Each sample has its own color described in the Sample_Annotation file, if it was not defined it is used the default palette coloring by condition. \n")
  cat("Two PCA are created, one based on the normalized protein intensities, tab **Proteins**. The second on the normalized peptide intensities, tab **Peptides** \n")
  cat(" \n \n")
...
if(deps_pep_l_df %>% dplyr::filter(class!="=") %>% nrow() > 0){
  ...
  suppressMessages(ggsave(paste0(dirOutput,"DEPs_peptides_PCA.pdf"), p1, device=cairo_pdf, width = (3+(0.13*nrow(pc))), height = (3+(0.07*nrow(pc))), units = c("in")))
...
doNextChunk <- tryCatch({
  ...
  writexl::write_xlsx((enr_df %>% dplyr::filter(`P<0.05` == "T",
...
if(p$is_alive() | is.null(p$get_result())){
  ...
  p$kill()
  #Save in RData for possible further analysis
  save(enr_df_universe, file=paste0(dirOutput,"DEPs_enrichment_data_universe.RData"))

  writexl::write_xlsx((enr_df_universe %>% dplyr::filter(`P<0.05` == "T",
                                                         overlap_size>=overlap_size_enrich_thr)),
                      path = paste0(dirOutput,"DEPs_enrichment_table_universe.xlsx"))
