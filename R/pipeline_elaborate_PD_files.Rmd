...
if(deps_l_df %>% dplyr::filter(class!="=") %>% nrow() > 0){
  ...
  pDEPs<-deps_b2b_lollipop(input_df=lolli_df, break_vec=seq(0,plyr::round_any(max(lolli_df$N)*1.1, if(max(lolli_df$N)>90){100}else{10}, f = ceiling),
                                                            round((plyr::round_any(max(lolli_df$N)*1.1, if(max(lolli_df$N)>90){100}else{10}, f = ceiling))/5, 0)), 
