...
doNextChunk <- tryCatch({
  ...
  enrich_df<-enr_df[which(enr_df$Significant == "TRUE" & enr_df$overlap_size>=overlap_size_enrich_thr),]
...
if(p$is_alive() | is.null(p$get_result())){
  ...
  p$kill()
  #Save in RData for possible further analysis
  enrich_all_df <- enr_df_universe %>% dplyr::filter(Significant == "TRUE", overlap_size>=overlap_size_enrich_thr)
