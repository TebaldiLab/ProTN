...
doNextChunk_STRING <- doNextChunk <- run_enrich <- run_STRING <- run_enrich_universe <- FALSE
...
if(deps_l_df %>% dplyr::filter(class!="=") %>% nrow() > 0){
  ...
  for(comp in unique(deps_l_df$comp)){
    ...
    cmd<-ggplot(data=input_df, aes(x=log2_FC, y=-log10(p_val), col=class, label=id)) +
      ...
      scale_y_continuous(limits = c(0, max(max(abs(-log10(input_df$p_val))),4))) +
      theme_bw(base_size = bs, base_family = bf)
...
p = callr::r_bg(
    func = function(g) {
      ...
      enrichRfnc_universe(g)
    },
    args = list(unique(psm_anno_df$symbol)),
    stdout = "|",
    stderr = "|",
...
doNextChunk <- tryCatch({
  ...
  writexl::write_xlsx((enr_df %>% dplyr::filter(fdr<=pval_enrich_thr,overlap_size>=overlap_size_enrich_thr)),
...
if(p$is_alive() | is.null(p$get_result())){
  ...
  writexl::write_xlsx((enr_df_universe %>% dplyr::filter(fdr<=pval_enrich_thr,overlap_size>=overlap_size_enrich_thr)),
