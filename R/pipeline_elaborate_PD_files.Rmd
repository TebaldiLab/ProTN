...
- **Tables** folder
  - ```normalised_intensities.xlsx```: excel file containing intensity values generated by proTN. Intensities are log2 transformed, normalized, imputed (and batch corrected). The file is organized in the following sheets:
    - **protein_per_sample**: protein intensities per sample.
    - **peptide_per_sample**: peptide intensities per sample.
    - **protein_per_condition**: protein intensities per condition (average & standard deviation), as defined in the Sample Annotation.
    - **peptide_per_condition**: peptide intensities per condition (average & standard deviation), as defined in the Sample Annotation.
  ...
  - ```differential_expression.xlsx```: excel file containing the results of differential analysis, according to the contrasts defined in the Design file. The file is organized in the following sheets:
    - **protein_DE**: protein differential expression results protein intensities per sample.
    ...
    - Columns for each contrast:
      - **class**: defined according to the fold change, p-value and intensity thresholds specified in the input
      ...
      - **log2_expr**: protein/peptide log2 average intensity
...
- **Figures** folder
  - ```PCA_MDS``` directory: Directory containing (Multidimensional scaling) and PCA (Principal component analysis) plots of samples.
    - **mds_proteins.pdf**: MDS, based on protein intensities.
...
cat("    - **selected_protein_plot.pdf**: plot displaying the intensities of selected proteins \n")
```
...
```{r print_descr_enrich_fig, warning=FALSE, echo=F, message=FALSE, eval=run_enrich, results='asis'}
...
cat("    - **enr_allproteins_keysources.pdf**: dot plot of top enriched terms based on all detected proteins. Terms are filtered for key datasets selected in the advanced options. \n")
cat("    - **enr_allproteins_keywords.pdf**: dot plot of top enriched terms based on all detected proteins. Terms are filtered for keywords defined in the advanced options. \n")
...
cat("#### 2.3.1. Batch correction {.tabset}  \n")
...
df_to_save<-list("protein_per_sample"=left_join(n_pep_prot,toPrint_datGene),
                 "protein_per_condition"=left_join(n_pep_prot,toPrint))
...
readme_sheet <- data.frame("INFO"=c(   NA,
                                ...
                                NA,
                                "In sheet *protein_per_sample* is written the normalized and imputed version of the original intensities of each protein in each sample.",
                                "In sheet *peptide_per_sample* is written the normalized and imputed version of the original intensities of each peptide in each sample.",
...
df_to_save<-list("README"=readme_sheet,
                 ...
                 "peptide_per_condition"=left_join(rownames_to_column(psm_peptide_table), 
                                                                              toPrint, by=c("rowname" = "id"))[,-c(1)])

writexl::write_xlsx(df_to_save,paste0(dirOutput,subfold_Tab,"normalised_intensities.xlsx"))
...
if(length(enrich_filter_term) > 0){
  ...
  tryCatch({
    ...
    ggsave(paste0(dirOutput_enr,"enr_allproteins_keysources.pdf"), el, device=cairo_pdf, width = 7, height = max(min(20,length(unique(enr_sele_df$anno_name))*0.5),3), units = c("in"))
  },
  error=function(cond){
    print("Error: No enriched term found with current parameters\n")
  })
...
if(length(enrich_filter_term) > 0){
  ...
  tryCatch({
    ...
    ggsave(paste0(dirOutput_enr,"enr_allProteins_keywords.pdf"), el, device=cairo_pdf, width = 7, height =
             max(min(20,length(unique(enr_sele_df$anno_name))*0.5),3), units = c("in"))
  },
  error=function(cond){
    print("Error: No enriched term found with current parameters\n")
