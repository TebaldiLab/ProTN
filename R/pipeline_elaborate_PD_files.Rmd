...
readTMT_files <- params$readTMT_files
...
#Clean files and merge
colToKeep<-intersect(colnames(input_files$Input), c("Condition","Color","Sample","MS_batch"))
input_files$Input <- input_files$Input[,colToKeep]
if(!("Condition" %in% colnames(input_files$Input))){ stop("Error: \'Condition\' column missin in \'Input\' file.") }
...
```{r load_data_from_TMT, eval=readTMT_files,echo=F, message=F, warning=F, results='asis'}
message("Reading files from MS...")
#Read all files in the input folder
template_files=c("Input","pep","prot")
filenames <- list("Input"=params$file_input,"prot"=params$file_prot,"pep"=params$file_pep)
...
#Clean files and merge
colToKeep<-intersect(colnames(input_files$Input), c("File Name","Condition","Color","Sample","MS_batch"))
input_files$Input <- input_files$Input[,colToKeep]
if(!("Condition" %in% colnames(input_files$Input))){ stop("Error: \'Condition\' column missin in \'Input\' file.") }
if(batch_corr_exe & !("MS_batch" %in% colnames(input_files$Input))){ stop("Error: \'MS_batch\' column missin in \'Input\' file with batch correction activated") }
# if("Sample" %in% colnames(input_files$Input)){input_files$Input$`File Name` <- input_files$Input$Sample}
...
#Verify if the Abundance start in 5 column in PEP table, if not remove extra column
# if(!grepl("Abundance",colnames(input_files$PEP)[5])){
input_files$PEP<-input_files$PEP[,c("Annotated Sequence",
                                    if("Modifications" %in% colnames(input_files$PEP)){"Modifications"}else{"Modifications in Master Proteins"},
                                    "Master Protein Accessions",
                                    "Positions in Master Proteins",
                                    colnames(input_files$PEP)[which(grepl("Abundances",
                                                                          colnames(input_files$PEP)))])]
# }

#merge by uniprot the prot description and the pep table
uniprot_to_take<-merge(input_files$PROT, input_files$PEP, by.x = "Accession", by.y = "Master Protein Accessions")
input_files$PD_PEP_matrix<-merge(input_files$PROT[which(input_files$PROT$Accession %in%
                                                          uniprot_to_take$Accession),],
                                 input_files$PEP, 
                                 by.x = "Accession", 
                                 by.y = "Master Protein Accessions")
input_files$PD_PEP_matrix<-input_files$PD_PEP_matrix[,c(c("Accession",
                                                          "Description",
                                                          "GeneName",
                                                          "Annotated Sequence",
                                                          if("Modifications" %in% colnames(input_files$PEP)){"Modifications"}else{"Modifications in Master Proteins"},
                                                          "Positions in Master Proteins"),
                                                        lapply(input_files$Input$`File Name`, function(x){
                                                          colnames(input_files$PD_PEP_matrix)[grepl(paste0(": ",x),
                                                                                                    colnames(input_files$PD_PEP_matrix))]
                                                          }) %>% unlist())]
...
#Save the different TMT from the colnames
# TMT_colnames <- data.table("columns"=grep("Abundance", colnames(input_files[["PD_PEP_matrix"]]), value = T))
# TMT_colnames[, split_col := tstrsplit(columns, "\\,", keep = 1)]
# TMT_colnames[, filename := tstrsplit(as.character(split_col), split="\\: ", keep = 2)
#              ][, TMT := tstrsplit(as.character(split_col), split="\\: ", keep = 3)]
# input_files[["Input"]] <- merge(input_files[["Input"]], 
#                                 TMT_colnames[,c("filename","TMT")], 
#                                 by.x = "File ID", by.y = "filename")
# input_files[["Input"]]$Sample <- paste(input_files[["Input"]]$Sample, input_files[["Input"]]$TMT, sep = "_")
# 
# colnames(input_files$PD_PEP_matrix)<-c(c("Accession","Description","GeneName","Annotated Sequence","Modifications","Position in Master Proteins"),
#                                        unlist(lapply(grep("Abundance:", colnames(input_files$PD_PEP_matrix), value = T), function(x){
#                                          tmp<-tstrsplit(x, "\\,", keep = 1)[[1]];
#                                          input_files$Input[which(input_files$Input$`File ID` %in% tstrsplit(as.character(tmp), 
#                                                                                                             split="\\: ", 
#                                                                                                             keep = 2)[[1]] &
#                                                                  input_files$Input$TMT %in% tstrsplit(as.character(tmp), 
#                                                                                                             split="\\: ", 
#                                                                                                             keep = 3)[[1]]), "Sample"]
#                                        })))

colnames(input_files$PD_PEP_matrix)<-c(c("Accession","Description","GeneName","Annotated Sequence","Modifications","Position in Master Proteins"), unlist(lapply(grep("Abundances", colnames(input_files$PD_PEP_matrix), value = T), function(x){
  col<- as.character(tstrsplit(x, "\\: ", keep = 2));
  input_files[["Input"]][which(input_files[["Input"]]$`File Name` == col), "Sample"]
})))
...
cat(" \n")
kable(c_anno[setdiff(colnames(c_anno), c("color","Color"))], row.names = F, col.names = str_to_title(setdiff(colnames(c_anno), c("color","Color"))))
cat(" \n")
```
...
hs<-ggplot(data=psm_long_df, aes(x=factor(sample, levels=unique(sample)),y=`log2 raw abundance`,fill=sample,colour=sample))+
  coord_flip()+
...
suppressMessages(suppressWarnings(hs))
...
hs<-ggplot(data=numeric_df, aes(x=factor(sample, levels=unique(sample)),y=`% of available abundances`,fill=sample,colour=sample))+
  coord_flip()+
...
cat("##### Proteins \n")
print(suppressMessages(suppressWarnings(hs)))
cat(" \n \n")
...
if(deps_l_df %>% dplyr::filter(class!="=") %>% nrow() > 0){
  ...
  pDEPs<-deps_b2b_lollipop(input_df=lolli_df, break_vec=seq(0,plyr::round_any(max(lolli_df$N)*1.2, if(max(lolli_df$N)>90){100}else{10}, f = ceiling),
                           ...
                           fill_vec = col_vec, color_vec = col_vec, char_max = 30, shape_vec = c(16,21), position_dodge = 0)
...
```{r cleaner, echo=F, message=F, warning=F, eval = doNextChunk_STRING, results='hide'}
...
#Mouse id --> 10090
  cat(" \n \n")
  cat("#### 4.2. Network plots  {.tabset}  \n")
  cat(" \n \n")
string_db <- STRINGdb$new(version="12", species=taxonomy_NCBI, score_threshold=500, input_directory="STRINGdb/")
for(comp in names(g_sel_comp)){
  
  # suppressWarnings(suppressMessages({
    gene_name<-g_sel_comp[[comp]] %>% unique() %>% as.data.frame()
    ...
    write_tsv(unique(string_gene_df), 
                file = paste0(dirOutput_net,gsub(comp, pattern = "\\/", replacement="vs"),"_connection.txt"))
    
    # string_db$plot_network(string_mapped)
    # string_db$get_link(string_ids = string_mapped$STRING_id, required_score = 500)
    
    link <- paste0("https://string-db.org/cgi/network?identifiers=",str_c(unique(string_mapped$STRING_id), collapse = "%0d"),"&species=",taxonomy_NCBI,"&required_score=500")
  # }))

  cat(" \n \n")
  cat("##### ",comp," \n")
  cat(paste0("To better visualize the network go to String-DB website --> [STRING-DB network](",link,") \n \n"))
  print((string_db$plot_network(string_mapped, required_score = 500)))
  cat(" \n \n")
...
```{r network_string, echo=F, message=F, warning=F, eval = F, fig.width=12, results='asis'}
