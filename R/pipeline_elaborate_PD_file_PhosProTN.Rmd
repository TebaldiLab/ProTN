...
filenames <- list("Input"=params$file_input_phos,"prot"=params$file_prot_phos,"pep"=params$file_pep_phos)
...
psm_sig_raw_prot$`Position in Master Proteins`<-gsub(" ", "_", psm_sig_raw_prot$`Position in Master Proteins`)
psm_sig_raw_prot$Modifications<-gsub(" ", "_", psm_sig_raw_prot$Modifications)
psm_sig_raw_phos$`Position in Master Proteins`<-gsub(" ", "_", psm_sig_raw_phos$`Position in Master Proteins`)
psm_sig_raw_phos$Modifications<-gsub(" ", "_", psm_sig_raw_phos$Modifications)
...
#   #For protein groups analysis
  psm_anno_raw_prot <- data.frame("symbol"=psm_sig_raw_prot$GeneName,
                                  "sequence"=psm_sig_raw_prot$`Annotated Sequence`,
                                  "modifications"=psm_sig_raw_prot$Modifications)
  psm_anno_raw_prot$id<-paste(psm_anno_raw_prot$symbol,psm_anno_raw_prot$sequence,psm_anno_raw_prot$modifications,sep="_")
  
  psm_anno_raw_phos <- data.frame("symbol"=psm_sig_raw_phos$GeneName,
                                  "sequence"=psm_sig_raw_phos$`Annotated Sequence`,
                                  "modifications"=psm_sig_raw_phos$Modifications)
...
# }
#PROTEOMICS
rownames(psm_sig_raw_prot)<-rownames(psm_anno_raw_prot)<-paste(psm_anno_raw_prot$symbol,seq(1:nrow(psm_anno_raw_prot)),sep="_")
...
rownames(psm_peptide_table_prot)<-make.names(paste(psm_peptide_table_prot$`Position in Master Proteins`,
                                        ...
                                        unique = T)
psm_sig_prot_raw_prot<-psm_sig_raw_prot[,c(7:ncol(psm_sig_raw_prot))]

#PHOSPHO
rownames(psm_sig_raw_phos)<-rownames(psm_anno_raw_phos)<-paste(psm_anno_raw_phos$symbol,seq(1:nrow(psm_anno_raw_phos)),sep="_")
...
rownames(psm_peptide_table_phos)<-make.names(paste(psm_peptide_table_phos$`Position in Master Proteins`,
                                        ...
                                        unique = T)
psm_sig_pet_raw_phos<-psm_sig_raw_phos[,c(7:ncol(psm_sig_raw_phos))]
...
psm_peptide_table<-psm_peptide_table_phos[filter_id_phos,]
...
#### 2.2. Percentage of numeric value in peptide intensity matrix for each sample {.tabset} 
...
dat_gene<-tryCatch(
  {
    ...
    grps <- as.factor(c_anno_prot$condition)
...
dat_pep<-tryCatch(
  {
    ...
    grps <- as.factor(c_anno_phos$condition)
    ppe <- scImpute(ppe, 0.6, grps)
    ppe <- tImpute(ppe, assay = "imputed")
...
#### 2.4. Normalized distributions {.tabset} 
...
suppressMessages(ggsave(paste0(dirOutput,"general_mds_phosphoproteome.pdf"), cmd, device=cairo_pdf, width = (3+(0.13*nrow(mds_cmdscale))), height = (3+(0.07*nrow(mds_cmdscale))), units = c("in")))
...
suppressMessages(ggsave(paste0(dirOutput,"general_mds_proteome.pdf"), cmd, device=cairo_pdf, width = (3+(0.13*nrow(mds_cmdscale))), height = (3+(0.07*nrow(mds_cmdscale))), units = c("in")))
...
suppressMessages(ggsave(paste0(dirOutput,"general_PCA_phosphoproteome.pdf"), p1, device=cairo_pdf, width = (3+(0.13*nrow(pc))), height = (3+(0.07*nrow(pc))), units = c("in")))
...
suppressMessages(ggsave(paste0(dirOutput,"general_PCA_proteome.pdf"), p1, device=cairo_pdf, width = (3+(0.13*nrow(pc))), height = (3+(0.07*nrow(pc))), units = c("in")))
...
suppressMessages(ggsave(paste0(dirOutput,"corrected_mds_phosphoproteomics.pdf"), cmd, device=cairo_pdf, width = 4, height = 3, units = c("in")))
...
suppressMessages(ggsave(paste0(dirOutput,"corrected_mds_proteome.pdf"), cmd, device=cairo_pdf, width = (3+(0.13*nrow(mds_cmdscale))), height = (3+(0.07*nrow(mds_cmdscale))), units = c("in")))
...
dat_pep$genename <- psm_peptide_table[rownames(dat_pep),"GeneName"]
dat_pep$names <- rownames(dat_pep)
dat_complete <- left_join(dat_pep, dat_gene)
...
expr_avgse_complete_df <- left_join(expr_avgse_pep_df, expr_avgse_df)
...
#Make the design
df <- data.frame("p" = sort(str_remove(c_anno_phos$condition, "_p\\b")), "i" = sort(str_remove(c_anno_prot$condition, "_i\\b")))
df <- unique(df[match(df$p, df$i),])
df$rule <- str_c(str_c(df$p, "_p"), "-", str_c(df$i, "_i"))
contro_list <- df$rule

contro_list<-c(contro_list,
               str_c(str_c("(", stringi::stri_replace_all_regex(formule_contrast, df$p, str_c(df$p, "_p"), vectorize = FALSE), ")"),
                     "-",
                     str_c("(", stringi::stri_replace_all_regex(formule_contrast, df$p, str_c(df$p, "_i"), vectorize = FALSE), ")"))
               )
names(contro_list) <- make.names(c(df$p, names(formule_contrast)), unique = T)

if(length(contro_list) > 0){
  deps_pep_df <- limmafnc("PEP",c_anno,dat_complete,psm_count_table,contro_list,expr_avgse_complete_df,signal_thr,fc_thr, pval_thr)
}
...
readme_sheet <- data.frame("INFO"=c(NA,
                                ...
                                "7. *class*: is “+” if the protein is up-regulated following the parameters set reported in the report.html, 
                                    “-” if the protein is down-regulated, “=” otherwise",
...
#Export table with SE e MEAN
df_to_save<-list("README"=readme_sheet,
                 "DEPs_PhosphoProteome"=left_join(rownames_to_column(psm_peptide_table), toPrint_pep, by=c("rowname" = "id"))[,-c(1)])
...
cat(" \n \n")
cat("#### Number of DEPs for each comparison \n")
...
```{r volcano_pep, fig.height=6, fig.width=6, warning=FALSE, echo=FALSE, results='asis', eval=FALSE}
# save(c_anno_prot,dat_gene,formule_contrast,expr_avgse_df,signal_thr,fc_thr, pval_thr, psm_peptide_table, psm_peptide_table_prot,
#      c_anno_phos,dat_pep,expr_avgse_pep_df, color_constrast, deps_pep_l_df, file = "tmp.RData")

col_sigle_contrast <- data.frame("color_constrast"=c_anno[match(df$p, str_remove(c_anno$condition, "_p\\b|_i\\b")),"color"], "color_constrast_dark"=c(""), row.names = df$p)
# darker.col = function(color, how.much = 50){
#     colorRampPalette(c(color, "black"))(100)[how.much]
# }
...
if(deps_pep_l_df %>% dplyr::filter(class!="=") %>% nrow() > 0){
  ...
  ggsave(paste0(dirOutput,"DEPs_mds_phosphoproteome.pdf"), cmd, device=cairo_pdf, width = (3+(0.13*nrow(mds_cmdscale))), height = (3+(0.07*nrow(mds_cmdscale))), units = c("in"))
...
if(deps_pep_l_df %>% dplyr::filter(class!="=") %>% nrow() > 0){
  ...
  suppressMessages(ggsave(paste0(dirOutput,"DEPs_phosphoproteome_PCA.pdf"), p1, device=cairo_pdf, width = (3+(0.13*nrow(pc))), height = (3+(0.07*nrow(pc))), units = c("in")))
...
# save(deps_pep_l_df, psm_peptide_table, df, pval_enrich_thr, overlap_size_enrich_thr, dirOutput, color_constrast, enrich_filter_DBs, file = "/home/gabriele/Desktop/UNITN/Proteomics_internship/ProTN/R/tmp.RData")
...
doNextChunk <- tryCatch({
  ...
  enr_df<-enrichRfnc(deps_pep_l_df[-which(deps_pep_l_df$comp %in% df$p),])
...
deps_l_df <- deps_pep_l_df[-which(deps_pep_l_df$comp %in% df$p),]
...
for (comp in names(doComp)){
  ...
  if(nrow(gene_vertices) != 0){
    ...
    layouts <- if(length(node_list$name) > 700){c("kk")}else{c("fr","kk")}
    p = callr::r_bg(
...
```{r kinase_tree_CORAL, echo=F, message=T, warning=F, eval=F, results='asis'}
shiny::setProgress(0.90, detail = "Network analysis in progress...")
cat(" \n \n")
cat("## 6. Kinase Trees \n")
cat("Kinase Tree for each condition made with PhosR and CORAL (http://phanstiel-lab.med.unc.edu/CORAL/) \n")

tmp_dat_pep <- dat_pep
rownames(tmp_dat_pep) <- make.names(psm_peptide_table[rownames(tmp_dat_pep), "GeneName"], unique = T)
# colnames(tmp_dat_pep) <- str_remove(colnames(tmp_dat_pep), "_p\\b")
for (comp in names(formule_CORAL)) {
  data("KinaseMotifs")
  ...
  GeneSymbol(ppe)<-psm_peptide_table$GeneName
  Sequence(ppe)<-psm_peptide_table$`Annotated Sequence`
  ...
  kinaseActivity<-(rowMeans(kssMat$ksActivityMatrix[, grepl("IL", colnames(kssMat$ksActivityMatrix))])-rowMeans(kssMat$ksActivityMatrix[, grepl("control", colnames(kssMat$ksActivityMatrix))]))[colnames(predMat)]
  write.table(kinaseActivity, file = "outputResults/KinaseActivity/t120/kinaseActivity_STK.txt")
}
