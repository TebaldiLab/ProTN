...
#### 1.4. Normalization, peptide-to-protein summarization, imputation 
...
suppressMessages(ggsave(paste0(dirOutput,subfold_Fig,subfold_MDSPCA,"mds_phosphoproteome.pdf"), cmd, device=cairo_pdf, width = (3+(0.13*nrow(mds_cmdscale))), height = (3+(0.07*nrow(mds_cmdscale))), units = c("in")))
...
suppressMessages(ggsave(paste0(dirOutput,subfold_Fig,subfold_MDSPCA,"mds_proteome.pdf"), cmd, device=cairo_pdf, width = (3+(0.13*nrow(mds_cmdscale))), height = (3+(0.07*nrow(mds_cmdscale))), units = c("in")))
...
suppressMessages(ggsave(paste0(dirOutput,subfold_Fig,subfold_MDSPCA,"pca_phosphoproteome.pdf"), p1, device=cairo_pdf, width = (3+(0.13*nrow(pc))), height = (3+(0.07*nrow(pc))), units = c("in")))
...
suppressMessages(ggsave(paste0(dirOutput,subfold_Fig,subfold_MDSPCA,"pca_proteome.pdf"), p1, device=cairo_pdf, width = (3+(0.13*nrow(pc))), height = (3+(0.07*nrow(pc))), units = c("in")))
...
## 2. Identification of Differentially Expressed Peptides of the phospho-proteomic

Differentially expressed peptides are identified using the Limma Bioconductor package. First are reported the comparison for which PhosProTN execute the differential analysis. Other than the formula desired by the user, for each condition is studied the differentiality between the phospho samples against the proteomic samples. 
...
#####
cat(" \n \n")
cat("#### 2.2. Number of differentially expressed peptides for each comparison  \n")
...
if(deps_pep_l_df %>% dplyr::filter(class!="=") %>% nrow() > 0){
  cat("#### 2.4. MDS based on differentially expressed peptides {.tabset}  \n " )
...
```{r pca_deps, warning=F, message=F, echo=F, results='asis'}
  cat(" \n \n")
  cat("#### 2.5. PCA based on differentially expressed peptides {.tabset}  \n")
