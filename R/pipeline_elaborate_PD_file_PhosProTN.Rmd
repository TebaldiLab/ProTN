...
contro_list<-formule_contrast[!unlist(lapply(str_split(formule_contrast, "\\(|\\)|\\-|\\+|\\*"), function(x) any(str_ends(x, "_i"))))]
if(length(contro_list) > 0){
  deps_pep_df <- limmafnc("PEP",c_anno_phos,dat_pep,psm_count_table,contro_list,expr_avgse_pep_df,signal_thr,fc_thr, pval_thr)
}
...
#Export table with SE e MEAN
df_to_save<-if(!is.null(toPrint)){ list("README"=readme_sheet,
                                        "DEPs_Proteome"=left_join(psm_peptide_table_prot, toPrint, by=c("GeneName" = "GeneSymbol")),
                                        "DEPs_PhosphoProteome"=left_join(rownames_to_column(psm_peptide_table), toPrint_pep, by=c("rowname" = "id"))[,-c(1)])} else {
                                         list("README"=readme_sheet,
