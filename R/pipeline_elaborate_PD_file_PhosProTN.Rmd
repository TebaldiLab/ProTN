...
read_input_files<-function(type, filenames){
  ...
  replacement <- lapply(str_extract_all(input_files$PEP$`Modifications in Master Proteins`, "\\(\\d+\\.\\d+|\\(\\d+"), 
                        function(x){as.integer(as.double(unlist(str_remove_all(unlist(x), "\\("))) > 75)})
  ...
  if("color" %in% colnames(c_anno)){
    ...
    c_anno$color<-colour_vec[match(c_anno$condition,unique(c_anno$condition))]
  }
  ...
  #READ PSM
    ...
    #                                                                            "\\: \\d+\\.\\d+\\b|\\: \\d+\\b"),
    #                                                            function(x){any(as.double(unlist(str_remove_all(unlist(x), "\\: "))) > 75)})),]
    # 
    # #Mantain only the first UNIPROT code
...
read_input_files<-function(type, filenames){
  ...
  if(type == "PHOS"){
    ...
    input_files$PEP <- input_files$PEP[unlist(lapply(str_extract_all(input_files$PEP$`Phospho (STY) Probabilities`, "1|(0\\.\\d+)"),
                                                     function(x){any(as.double(unlist(x)) > 0.75)})),]
    #Modify sequence to detect only the phosphorilated sites
    ...
    replacement <- lapply(str_extract_all(input_files$PEP$`Phospho (STY) Probabilities`, "1|(0\\.\\d+)"), 
                          function(x){as.integer(as.double(unlist(x)) > 0.75)})
...
cat("\n \n")
if(readPD_files){
  active_sites <- lapply(lapply(stri_extract_all_regex(psm_peptide_table$modif, "\\w\\d+\\(1|\\w\\d+\\.|\\ \\w\\]|\\w\\/\\w|\\w\\/\\w\\/\\w"), 
...
cat("#### 2.8. Batch correction {.tabset}  \n")
...
names(contro_list) <- make.names(c(str_c(df$p,"_default"), names(formule_contrast)), unique = T)
...
doNextChunk <- tryCatch({
  ...
  enr_df<-enrichRfnc(deps_pep_l_df[which(deps_pep_l_df$comp %in% names(formule_CORAL)),])
...
```{r kinase_tree_CORAL, echo=F, message=T, warning=F, eval=(run_kinaseTree & run_enrich), results='asis'}
